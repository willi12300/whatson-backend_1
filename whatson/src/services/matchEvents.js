...
async function matchEventToVenue(event, city) {
  ...
  if (event.venueLat && event.venueLng) {
    const { rows } = await query(
      `SELECT id, name, lat, lng FROM venues WHERE city = $1 AND lat BETWEEN $2 - 0.01 AND $2 + 0.01 AND lng BETWEEN $3 - 0.015 AND $3 + 0.015`,
    ...
    for (const v of rows) {
      ...
      const combined = nameSim * 0.7 + Math.max(0, 1 - dist / 250) * 0.3
      if (nameSim >= 0.7 && combined > bestScore) { bestScore = combined; best = v }
    }
  ...
  if (target.length > 3) {
    const { rows } = await query(
      `SELECT id, name FROM venues WHERE city = $1 AND similarity(normalised_name, $2) > 0.4 ORDER BY similarity(normalised_name, $2) DESC LIMIT 1`,
      [city, target]
    )
    if (rows.length) {
